...
@interface KYEWebCacheHandlerImpl ()

@property (nonatomic, strong) NSCache *memCache;
@property (nonatomic, strong) NSString *dirPath;
@end

@implementation KYEWebCacheHandlerImpl
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.dirPath = [paths.firstObject stringByAppendingPathComponent:@"kye.web.default.cache"];
    }
    return self;
}
...
- (void)setData:(KYEWebDataModel *)data forKey:(NSString *)key
{
...
- (KYEWebDataModel *)dataForKey:(NSString *)key
{
    KYEWebDataModel *data = [self.memCache objectForKey:key];
    if (!data) {
        NSString *filePath = [self filePathForKey:key];
