...
(function() {
 if (window.kye_realxhr) {
 return
 }
 window.kye_realxhr = XMLHttpRequest;
 var timestamp = new Date().getTime();
 timestamp = parseInt((timestamp / 1000) % 100000);
 var global_index = timestamp + 1;
 var global_map = {};
 window.kye_realxhr_callback = function(id, message) {
 ...
 BaseHookAjax.prototype = window.kye_realxhr;
 ...
 var xhr = new window.kye_realxhr();
 this._xhr = xhr;
 var that = this;
 ...
 window.webkit.messageHandlers.KYEXHR.postMessage(message)
 };
 ...
 window.kye_hookAjax = function() {
 XMLHttpRequest = hookAjax
 };
 window.kye_unhookAjax = function() {
 XMLHttpRequest = window.kye_realxhr
 }
 })();
